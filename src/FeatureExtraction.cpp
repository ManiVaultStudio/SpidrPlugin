...
void FeatureExtraction::extractFeatures() {
    ...
    //// convolve over all data points
    //std::vector<int> p(numPoints);
    //std::iota(std::begin(p), std::end(p), 0);
    //std::vector<float>::iterator dataIt = _inputData.getDataNonConst().begin();
    //std::for_each(std::execution::par_unseq, std::begin(p), std::end(p), [data, dataIt, numDims, numPoints](int pointID) {
    //    // step forward to next point
    //    std::advance(dataIt, pointID * numDims);
